...
function exc.catchraw(func, handlers, passArgs, finalFunc)
	assert(type(func) == "function", "u dumbass")
	local r
	local res = {xpcall(func, function(err)
		r = {err, debug.traceback()}
		return r
	end)}
	...
	if not res[1] then
		if not r then
			r = {"nil", tostring(res[2])}
		end

		if type(r) ~= "table" then
			error("what the fuck " .. type(r) .. " \"" .. tostring(r) .. "\"")
		end

		local err, tb = r[1], r[2]
...
function exc.xpcall(func, cb, ...)
	return true, exc.catch(func, {...}, "*", function(err)
		if cb then
...
function exc.pcall(func, ...)
	...
	return exc.xpcall(function()
		return true, func(unpack(params))
	end, function(err)
		return false, err
	end)
end
